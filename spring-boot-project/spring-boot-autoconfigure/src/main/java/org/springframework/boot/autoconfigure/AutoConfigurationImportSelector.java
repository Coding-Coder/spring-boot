...
public class AutoConfigurationImportSelector implements DeferredImportSelector, BeanClassLoaderAware,
	...
	// AutoConfigurationImportSelector 实现了很多其它的 Aware 接口，分别表示在某个时机会被回调，然后将对应的对象赋值给下面的4个成员变量。
	...
	protected AutoConfigurationEntry getAutoConfigurationEntry(AutoConfigurationMetadata autoConfigurationMetadata,
		...
		// 检查要被排除的配置类，因为@SpringBootApplication(exclude = FreeMarkerAutoConfiguration.class)配置时，可能配置的不是自动配置类，故要抛出异常
	...
	protected List<String> getCandidateConfigurations(AnnotationMetadata metadata, AnnotationAttributes attributes) {
		...
		// 通过指定的 beanClassLoader 从 META-INF/spring.factories 的资源文件中，
		// 读取 key 为 EnableAutoConfiguration 的 value 值，赋值给 configurations
	...
	private List<String> filter(List<String> configurations, AutoConfigurationMetadata autoConfigurationMetadata) {
		...
		for (AutoConfigurationImportFilter filter : getAutoConfigurationImportFilters()) {
			...
			/*************************【主线，重点关注】*****************************/
			// 判断各种filter来判断每个candidate（这里实质要通过candidate(自动配置类)拿到其标注的@ConditionalOnClass,@ConditionalOnBean和@ConditionalOnWebApplication里面的注解值）是否匹配，
			// 注意match顺序跟candidates的自动配置类一一对应
	...
	private static class AutoConfigurationGroup
		...
		// 这里用来处理自动配置类,比如过滤掉不符合匹配条件的自动配置类,只是对数据进行加载
		@Override
